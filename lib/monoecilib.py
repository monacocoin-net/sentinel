...
def is_valid_monoeci_address(address, network='mainnet'):
    ...
    monoeci_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Monoeci address
        return False

    if (address_version != monoeci_version):
        return False
...
# shims 'til we can fix the monoecid side
def SHIM_serialise_for_monoecid(sentinel_hex):
    ...
    # superblock => "trigger" in monoecid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # monoecid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    monoecid_hex = serialise(obj)
    return monoecid_hex


# shims 'til we can fix the monoecid side
def SHIM_deserialise_from_monoecid(monoecid_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(monoecid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('monoeci.conf')
