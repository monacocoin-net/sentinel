...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, monoecid):
        golist = monoecid.rpc_command('gobject', 'list')
    ...
    def import_gobject_from_monoecid(self, monoecid, rec):
        ...
        # shim/monoecid conversion
        object_hex = monoecilib.SHIM_deserialise_from_monoecid(object_hex)
        objects = monoecilib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from monoecid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from monoecid, with every run
    ...
    def vote(self, monoecid, signal, outcome):
        import monoecilib
        ...
        output = monoecid.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = monoecilib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(monoecid, signal)

    def sync_network_vote(self, monoecid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = monoecid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def get_prepare_command(self):
        import monoecilib
        obj_data = monoecilib.SHIM_serialise_for_monoecid(self.serialise())
    ...
    def prepare(self, monoecid):
        try:
            object_hash = monoecid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def hash(self):
        import monoecilib
        return monoecilib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, monoecid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - monoecid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, monoecid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - monoecid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, monoecid):
        now = int(time.time())
        return (self.created_at < (now - monoecid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, monoecid):
        if self.is_expired(monoecid):
            return False

        return True

    def is_deletable(self, monoecid):
        if self.is_expired(monoecid):
            return True
