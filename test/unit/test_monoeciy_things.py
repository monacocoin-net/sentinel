...
def test_valid_monoeci_address():
    from monoecilib import is_valid_monoeci_address

    main = valid_monoeci_address()
    test = valid_monoeci_address('testnet')

    assert is_valid_monoeci_address(main) is True
    assert is_valid_monoeci_address(main, 'mainnet') is True
    ...
    assert is_valid_monoeci_address(test, 'testnet') is True
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from monoecilib import elect_mn
...
def test_parse_masternode_status_vin():
    from monoecilib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % monoecilib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(monoecilib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(monoecilib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(monoecilib.blocks_to_seconds(16616)) == 2612035
